...
class Robot(object):
  ...
  def add_obs_constraint(self, prog, x, q_avoid_list, N):
        limit_val = np.arctan2(.125,.5) # Obstacle is cube of side 0.25 and 0.5m away from robot in XY plane. This helps maintain a buffer and cover all possible configurations in the 7 dimensional hyperplane.
  ...
  def compute_mpc_feedback(self, x_current, x_final, time, tf):
    ...
    if avoid_obs:
        with open("constraints450_q.pkl", 'rb') as f:
            q_avoid_list = pickle.load(f)
        # print(len(q_avoid_list))
        self.add_obs_constraint(prog, x, q_avoid_list, N)
    ...
    # print("result.get_optimal_cost(): ",result.get_optimal_cost())
    # print("result.get_solution_result(): ", result.get_solution_result())
    # raise
